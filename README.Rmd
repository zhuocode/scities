...
Once installed, the packages do not need to installed again for subsequent use. The can simply be called using the `library` function. Load packages at the start of your R session or scripts as follows:
...
To download data used in this session go to https://github.com/ITSLeeds/scities/releases and click on [**data_sci.zip**](https://github.com/ITSLeeds/scities/releases/download/0.1/data_sci.zip) (or just click on that link).

Unzip the folder.
You should now have a folder called `data_sci` on your computer.
Navigate into it and open `.data_sci.Rproj`.
...
The origin-destination data in `input` can be converted into a spatial (`sf`) object as follows:
...
`input_sf` is a fully fledged spatial object so we can do geographic operations on it, such as finding a 500 m buffer around each one.
Instead of putting a buffer around *every* flow we will pick the top 100 lines for clarity:
